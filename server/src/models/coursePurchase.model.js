...
import { CoursePurchasePaymentEnum } from "../constant.js";
...
const coursePurchaseSchema = new mongoose.Schema(
  {
    ...
    status: {
      type: String,
      enum: CoursePurchasePaymentEnum,
      default: CoursePurchasePaymentEnum.PENDING,
    },
    paymentMethod: {
      type: String,
...
coursePurchaseSchema.virtual("isRefundable").get(function () {
  if (this.status !== CoursePurchasePaymentEnum.COMPLETED) return false;
...
coursePurchaseSchema.methods.processRefund = async function (reason, amount) {
  this.status = CoursePurchasePaymentEnum.REFUNDED;
